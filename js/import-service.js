(function () {
  ...
  const TEMPLATE_HEADERS = [
    ...
    "first_due_date", "grace_period_days", "expected_payment_method"
  ];
  ...
  function validateRows(rows) {
    ...
    rows.forEach((row) => {
      ...
      if (row.first_due_date && !validDate(row.first_due_date)) {
        errors.push({ row: row.__row, message: "first_due_date must use YYYY-MM-DD format." });
      }
      if (row.grace_period_days && (!Number.isFinite(Number(row.grace_period_days)) || Number(row.grace_period_days) < 0)) {
        errors.push({ row: row.__row, message: "grace_period_days must be zero or a positive number." });
      }
    ...
    return [
      {
        ...
        first_due_date: "2026-01-01", grace_period_days: "0", expected_payment_method: "Post-Dated Cheque"
      },
      {
        property_code: "R16", property_name: "Marina Tower 16", property_type: "Mixed-use tower", property_location: "Doha",
      ...
      },
      {
        ...
        first_due_date: "2026-09-01", grace_period_days: "0", expected_payment_method: "Post-Dated Cheque"
      }
    ];
  }
