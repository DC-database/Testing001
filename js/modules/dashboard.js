(function () {
  ...
  async function render(view) {
    ...
    view.innerHTML = `
      <section class="dashboard-hero">
        <div class="dashboard-hero-copy">
          <div class="live-eyebrow"><span></span> LIVE PORTFOLIO · LOCAL DEMO</div>
          <h1>Portfolio Overview</h1>
          <p>Every figure is calculated from the active property, unit and contract records—never from manually typed dashboard totals.</p>
        </div>
      ...
      <section class="dashboard-section">
        <header class="dashboard-section-header"><div><span>PROPERTY PERFORMANCE</span><h2>One clear view of every asset.</h2></div><button class="text-button" data-route-properties>Open property collection →</button></header>
      ...
      <section class="dashboard-section">
        <header class="dashboard-section-header"><div><span>LEASE EXPIRY CALENDAR</span><h2>Next 12 months of revenue risk.</h2></div><button class="text-button" data-open-report="lease-expiry">Detailed lease report →</button></header>
      ...
      <section class="dashboard-section action-section">
        <header class="dashboard-section-header"><div><span>ACTION REQUIRED</span><h2>Items that need the manager’s attention.</h2></div><button class="text-button" data-open-actions>Open Action Center →</button></header>
    ...
    view.querySelectorAll("[data-kpi]").forEach((el) => el.addEventListener("click", () => {
      ...
      } else if (key === "expiring") root.router.navigate("contracts", { status: "expiring" });
    }));
  }
