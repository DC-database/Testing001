(function () {
  ...
  async function render(view) {
    if (!root.auth.can("import")) {
      root.ui.toast("Administrator access is required for portfolio import.", "error");
      return root.router.navigate(root.state.user?.role === "ceo" ? "overview" : "properties");
    }
    currentRows = [];
    ...
    view.innerHTML = `
      ...
      <section class="import-layout">
        <article class="panel import-control-panel">
          <header class="panel-header"><div><h3>Select portfolio CSV</h3><p>The original file remains on your computer. Nothing leaves this browser while local storage is active.</p></div></header>
      ...
      <section class="panel import-field-guide">
        ...
        <div class="panel-body guide-grid">
          <div><span>UNIT STATUS</span><strong>occupied · booked · vacant · inspection · maintenance · renovation · unavailable</strong></div>
          ...
          <div><span>EMPTY / WORK STATUS</span><strong>For vacant, inspection, maintenance, renovation or unavailable units, leave tenant and contract columns blank</strong></div>
          ...
          <div><span>PAYMENT SCHEDULE</span><strong>Use first_due_date, grace_period_days and expected_payment_method; actual paid dates never shift future due dates</strong></div>
        </div>
      </section>`;
    ...
    view.querySelector("[data-import-button]").addEventListener("click", async () => {
      ...
      try {
        ...
        await root.router.navigate("overview");
      } catch (error) {
